...
start_broken_folder = '/Users/simyoonchan/Documents/EncodingProject/BrokenRound1-Test2'
...
def encode_files_as_word_doc():
    for root_folder, _, list_of_file_names in os.walk(destination_resolved_folder):
        for file in list_of_file_names:
            ...
            if content:
                # Sanitize the content to remove problematic characters
                content = re.sub(r'[^\x20-\x7E]', '', content)  # Remove non-ASCII characters

                doc = Document()
                doc.add_paragraph(content.encode('ascii', 'ignore').decode('utf-8'))  # Remove non-ASCII characters
                ...
                print(f'Unable to decode {broken_file_start_file_path} using the detected encoding')

                # If decoding with the detected encoding fails, try fallback encodings
                for fallback_encoding in list_fallback_encodings:
                    try:
                        ...
                        if content:
                            ...
                            doc.save(destination_docname_file_path)
                            os.remove(broken_file_start_file_path)
                            print(f'Successfully decoded with fallback encoding ({fallback_encoding}) and saved as {destination_docname_file_path}')
                            break  # Exit the loop if successful with a fallback encoding
                    except UnicodeDecodeError:
                        continue

                if not content:
                    print(f'Unable to decode {broken_file_start_file_path} with any of the specified encodings')
...
#What to input into Command Prompt in MacBook:
    ...
    #2) python3 encode_files_edit3.py 
